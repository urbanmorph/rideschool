...
def downgrade() -> None:
    with op.batch_alter_table('organisation') as batch_op:
        ...
        batch_op.execute('''
            CREATE OR REPLACE FUNCTION public.update_organisation_updated()
            RETURNS TRIGGER AS $$
            BEGIN
            ...
            CREATE TRIGGER update_organisation_updated
            BEFORE UPDATE 
            ON public.organisation
            FOR EACH ROW
            EXECUTE FUNCTION public.update_organisation_updated();
        ''')